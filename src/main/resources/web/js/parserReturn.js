    ...
    porserType2Item(item){
        ...
        if(item.result){
            ...
            if(result.value==='Success'){
                ...
                messageShowManager.addError(result.message);
                messageShowManager.addError('24消息请求数达到了限制！');
            }else{
                messageShowManager.addError(result.message);
                messageShowManager.addError('发生未知错误！');
            }
        }
        if (item.throttling) {
    ...
    porserMessages(messages, father) {
        for (let i = 0; i < messages.length; i++) {
            ...
            if (!message.messageType && message.adaptiveCards) {//如果是正常的聊天
                ...
                let div = this.getByID(message.messageId, 'div', chat, 'error');
                div.innerHTML = `
            ${message.hiddenText}<br>聊天中断！试试开始新主题？
            `;

            }else if(message.contentOrigin === 'TurnLimiter'){
                messageShowManager.addError(message.text);
                messageShowManager.addError('聊天被限制了，试试开始新主题？');
    ...
    */
    ...
    async generateContentQueryImg(message, father){
        ...
        try{
            ...
            if(!urr || !urr[1]){
                console.log(html);
                messageShowManager.addError("请求图片返回不正确的页面，无法加载图片。");
                return;
            }
            let ur = urr[1];
            ur = ur.replaceAll('&amp;','&');
            ...
            console.error(e);
            messageShowManager.addError("请求图片失败:"+e);
        }
    }
...
let parserReturnMessage = new ParserReturnMessage();
